...
function addEventListeners() {
    ...
    if (availabilityFilter) {
        availabilityFilter.addEventListener('change', () => {
            currentAvailabilityFilter = availabilityFilter.value;
            filterMenuItems();
        });
    }
...
function displayMenuItems(items) {
    ...
    if (itemsCount) {
        itemsCount.textContent = `Showing ${items.length} ${items.length === 1 ? 'item' : 'items'}`;
    }
    ...
    sortedItems.forEach(item => {
        ...
        const availabilityStatus = item.availability || 'available';
        const availabilityBadge = availabilityStatus === 'available' 
            ? '<span class="status-badge available">Ready to serve</span>' 
...
function openItemModal(item = null) {
    ...
    if (item) {
        ...
        document.getElementById('item-availability').value = item.availability || 'available';
...
function clearAllFilters() {
    ...
    // Re-apply filters (which will now be the defaults)
    filterMenuItems();
}
